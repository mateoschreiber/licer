...
export class TenderBranchesController {
  constructor(private readonly service: TenderBranchesService) {}
  @Permissions('tenders:read:internal') @Get() findAll() {
    return this.service.findAll();
  }
  @Permissions('tenders:create:internal') @Post() create(@Body() dto: CreateTenderBranchDto) {
    return this.service.create(dto);
  }
  @Permissions('tenders:update:internal') @Patch(':id') update(
    @Param('id') id: string,
    @Body() dto: CreateTenderBranchDto,
  ) {
