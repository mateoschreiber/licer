...
export function UsersRolesPage() {
  ...
  function startRoleEdit(role: RoleRow) {
    ...
    roleForm.reset({
      ...
      permissionIds:
        (role.permissions?.map((item) => item.permission?.id).filter(Boolean) as string[]) ?? [],
    });
  }
        ...
        <section className="admin-crud-grid">
          ...
          <section className="panel admin-form-panel">
            ...
            >
              ...
              <label>
                Usuario
                <input {...userForm.register('username', { required: true, minLength: 3 })} />
              </label>
              <label>
                Nombre
              ...
              <label>
                ...
                <select
                  multiple
                  size={Math.min(Math.max(roles.length, 4), 8)}
                  {...userForm.register('roleIds', { required: true })}
                >
                  {roles.map((role) => (
        ...
        <section className="admin-crud-grid">
          <div className="panel admin-list-panel">
            <div className="section-heading">
              <div>
                <h2>Roles</h2>
                <p>Administre los permisos por rol.</p>
              </div>
                ...
                },
                {
                  ...
                  render: (row) => (
                    <button
                      className="button ghost"
                      type="button"
                      onClick={() => startRoleEdit(row as RoleRow)}
                    >
                      <Pencil size={16} /> Editar
                    </button>
                  ),
          ...
          <section className="panel admin-form-panel">
            <div className="section-heading">
              <div>
                ...
                <p>{editingRole ? editingRole.name : 'Defina nombre y permisos.'}</p>
              </div>
            </div>
            <form
              className="stack-form"
              onSubmit={roleForm.handleSubmit((values) => saveRole.mutate(values))}
            >
              <label>
                Nombre del rol
              ...
              <label>
                ...
                <select
                  multiple
                  size={Math.min(Math.max(permissionOptions.length, 6), 12)}
                  {...roleForm.register('permissionIds', { required: true })}
                >
              ...
              <div className="form-actions">
                <button
                  ...
                  type="submit"
                  disabled={saveRole.isPending || permissionOptions.length === 0}
                >
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link className="button primary" to="/supplier/register">
            <Plus size={16} /> Anadir proveedor
          </Link>
        }
      />
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            header: 'Acciones',
            render: (row) => (
              <Link className="button ghost" to={`/internal/suppliers/${String(row.id)}`}>
                Ver detalles
              </Link>
            ),
          },
...
export function SupplierDetailPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => api.get<SupplierAdminDetail>('/suppliers/' + id),
    enabled: Boolean(id),
  });
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      legalRepresentativeFirstName:
        ...
        (data.legalRepresentative ?? '').trim().split(/\s+/)[0] ??
        '',
      legalRepresentativeLastName:
        data.legalRepresentativeLastName ??
        (data.legalRepresentative ?? '').trim().split(/\s+/).slice(1).join(' '),
  ...
  const addDocument = useMutation({
    mutationFn: async () => {
      ...
      return api.post('/suppliers/' + id + '/documents', {
        fileId: uploaded.id,
        type: documentType,
  ...
  const approveDocument = useMutation({
    mutationFn: (documentId: string) =>
      api.patch('/suppliers/' + id + '/documents/' + documentId, { status: 'APROBADO' }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['supplier-detail', id] }),
  });
  const deleteDocument = useMutation({
    mutationFn: (documentId: string) => api.delete('/suppliers/' + id + '/documents/' + documentId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['supplier-detail', id] }),
  });
  const deleteSupplier = useMutation({
    mutationFn: () => api.delete('/suppliers/' + id),
    onSuccess: () => navigate('/internal/suppliers'),
  });

  return (
    <>
      <PageHeader
        title={data ? data.legalName : 'Detalle proveedor'}
        description={data ? data.ruc : ''}
        actions={
          <div className="row-actions">
            <button
              className="button ghost"
              type="button"
              onClick={() => navigate('/internal/suppliers')}
            >
              Volver
            </button>
            <button
        ...
        <>
          {!isAdmin ? (
            <section className="panel">
              <p>Solo el administrador puede modificar datos y documentos de proveedores.</p>
            </section>
          ) : (
            <>
              <section className="panel">
                ...
                >
                  <label>
                    ...
                    <input
                      inputMode="numeric"
                      autoComplete="off"
                      {...form.register('ruc', { required: true })}
                    />
                  </label>
                  <label>
                    Razon social
                  ...
                  <label>
                    ...
                    <input
                      inputMode="numeric"
                      autoComplete="off"
                      {...form.register('legalRepresentativeDocumentId')}
                    />
                  </label>
                  <label>
                    Correo de contacto
              ...
              <section className="panel">
                <div className="section-heading">
                  <div>
                    ...
                    <p>
                      Constancia de RUC: obligatoria. Nota de conformidad: deseable. Logo: foto de
                      perfil.
                    </p>
                  </div>
                </div>
                    ...
                    render: (row) => (
                      <div className="row-actions">
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            void previewFile('/files/' + String(row.fileId) + '/download')
                          }
                        >
                          Previsualizar
                        </button>
                        {String(row.status) !== 'APROBADO' && (
                          <button
                            className="button primary"
                            type="button"
                            onClick={() => approveDocument.mutate(String(row.id))}
                          >
...
function TenderAttachmentPreview({
  document,
}: {
  document: { id: string; title: string; type: string; fileId: string };
}) {
  ...
  useEffect(() => {
    ...
    fetch(API_URL + '/files/' + document.fileId + '/download', {
  ...
  return (
    <li className="attachment-preview">
      {previewUrl ? <img src={previewUrl} alt={document.title} /> : <span>{document.title}</span>}
      <small>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'deadline', header: 'Cierre', render: (row) => formatPyDate(row.bidDeadline) },
          {
            key: 'action',
            header: '',
            render: (row) => <Link to={`/internal/tenders/${row.id}`}>Abrir</Link>,
          },
        ]}
      />
    </>
...
const emptyTenderItem: TenderLineItem = {
  ...
  minimumWarranty: '',
  warrantyDocumentRequired: false,
};
...
export function TenderCreateEditPage() {
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: () => api.get<Array<{ id: string; name: string }>>('/tender-categories'),
  });
  const { data: branches = [] } = useQuery({
    queryKey: ['tender-branches'],
    queryFn: () => api.get<Array<{ id: string; name: string }>>('/tender-branches'),
  });

  const { data: areas = [] } = useQuery({
    queryKey: ['requesting-areas'],
    queryFn: () =>
      api.get<Array<{ id: string; code: string; name: string; status: string }>>(
        '/requesting-areas',
      ),
  });
  ...
  useEffect(() => {
    ...
    if (!questionEdited) {
      setValue(
        'questionDeadline',
        toDateTimeLocal(new Date(base.getTime() + 15 * 24 * 60 * 60 * 1000)),
      );
    }
    if (!bidEdited) {
    ...
    try {
      const tender = await api.post<TenderSummary>('/tenders', {
        ...
        questionDeadline: values.questionDeadline
          ...
          : undefined,
        bidDeadline: values.bidDeadline ? new Date(values.bidDeadline).toISOString() : undefined,
      });
      await Promise.all(
        validItems.map((item) =>
          api.post(`/tenders/${tender.id}/items`, {
      ...
      const uploadDocuments = async (files: File[], type: 'TECNICO' | 'ANEXO') => {
        for (const file of files) {
          ...
          const uploaded = await apiRequest<{ id: string }>('/files/upload', {
            method: 'POST',
            body: data,
          });
          await api.post('/tender-documents', {
            tenderId: tender.id,
            type,
            title: file.name,
            fileId: uploaded.id,
      ...
      <section className="panel">
        <form
          ...
          onSubmit={handleSubmit(onSubmit, () =>
            setSubmitError('Complete todos los campos obligatorios antes de guardar.'),
          )}
        >
          <label>
            Codigo
          ...
          <label>
            Limite respuestas
            <input type="date" {...register('responseDeadline', { required: true })} />
          </label>
          <label>
            Limite ofertas
            <input
            ...
            <div className="line-items-table">
              ...
              {items.map((item, index) => (
                <div className="line-items-row" key={index}>
                  ...
                  <input
                    value={item.specs}
                    onChange={(event) => updateItem(index, 'specs', event.target.value)}
                    placeholder="Especificaciones"
                  />
                  <input
                    value={item.referenceBrandModel}
                    onChange={(event) =>
                      updateItem(index, 'referenceBrandModel', event.target.value)
                    }
                  ...
                  <input
                    ...
                    onChange={(event) => updateItem(index, 'minimumWarranty', event.target.value)}
                    placeholder="Ej.: 6 meses"
                  />
                  <label className="item-check">
                    <input
                      type="checkbox"
                      checked={item.warrantyDocumentRequired}
                      onChange={(event) =>
          ...
          <div className="full commercial-conditions">
            <h2>Condiciones comerciales</h2>
            <div className="commercial-row">
              <fieldset className="currency-options">
                <legend>Moneda</legend>
                <label>
                  <input type="radio" value="PYG" {...register('currency')} /> Gs.
                </label>
                <label>
                  <input type="radio" value="USD" {...register('currency')} /> USD
                </label>
              </fieldset>
              <label className="commercial-check">
                <input type="checkbox" defaultChecked {...register('vatIncluded')} />
              ...
              {watch('paymentMethod') === 'CREDITO' && (
                <>
                  ...
                  {paymentTerms === 'PERSONALIZADO' && (
                    <label>
                      Dias personalizados
                      <input type="number" min="1" {...register('customCreditDays')} />
                    </label>
                  )}
                </>
              )}
          ...
          <div className="full">
            ...
            {planFiles.length > 0 && (
              <small className="selected-files">
                {planFiles.length} archivo(s) seleccionado(s):{' '}
                {planFiles.map((file) => file.name).join(', ')}
              </small>
            )}
          </div>
          {submitError && (
            <p className="error-message full" role="alert">
              {submitError}
            </p>
          )}
          <div className="form-actions full">
            ...
            <button
              className="button ghost"
              type="button"
              onClick={() => navigate('/internal/tenders')}
            >
              Cancelar
            </button>
          </div>
...
export function TenderDetailInternalPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => api.get<TenderDetailData>('/tenders/' + id),
    enabled: Boolean(id),
  });
  ...
  const removeTender = useMutation({
    mutationFn: () => api.delete('/tenders/' + id),
    onSuccess: () => navigate('/internal/tenders'),
  });
  return (
    <>
      <PageHeader
        ...
        actions={
          <div className="row-actions">
            <button
              className="button ghost"
              type="button"
              onClick={() => data && previewTenderA4(data)}
            >
              Previsualizar
            </button>
            <button
      ...
      {!data ? (
        ...
        <>
          ...
          <section className="panel tender-detail">
            ...
            <dl className="detail-grid">
              <dt>Fecha base</dt>
              <dd>{formatPyDate(data.publishedAt)}</dd>
              <dt>Limite de consultas</dt>
              <dd>{formatPyDate(data.questionDeadline)}</dd>
              <dt>Limite de respuestas</dt>
              <dd>{formatPyDate(data.responseDeadline)}</dd>
              <dt>Limite de ofertas</dt>
              <dd>{formatPyDate(data.bidDeadline)}</dd>
            </dl>
          </section>
          <section className="panel tender-detail">
            <h2>Condiciones comerciales</h2>
          ...
          <section className="panel tender-detail">
            ...
            {data.documents?.length ? (
              <ul className="attachment-list attachment-gallery">
                {data.documents.map((doc) => (
                  <TenderAttachmentPreview key={doc.id} document={doc} />
                ))}
              </ul>
            ) : (
          ...
          },
          {
            ...
            render: (row) => (
              <Link
                ...
                to={'/internal/questions/' + String(row.id)}
              >
                Abrir
              </Link>
            ),
...
interface QuestionTicket {
  ...
  supplier?: { legalName: string; ruc: string };
...
export function QuestionDetailInternalPage() {
  ...
  const { data } = useQuery({
    queryKey: ['internal-question', id],
    queryFn: () => api.get<QuestionTicket>('/questions/' + id),
    enabled: Boolean(id),
  });
  const answer = useMutation({
    mutationFn: (values: { text: string }) => api.post('/questions/' + id + '/answer', values),
    onSuccess: async () => {
      form.reset();
      await queryClient.invalidateQueries({ queryKey: ['internal-question', id] });
      await queryClient.invalidateQueries({ queryKey: ['internal-questions'] });
    },
  });
  return (
    <>
      <PageHeader
        ...
        actions={
          <button
            className="button ghost"
            type="button"
            onClick={() => navigate('/internal/questions')}
          >
            Volver
          </button>
        }
      ...
      {!data ? (
        ...
        <section className="panel ticket">
          <div className="ticket-head">
            <div>
              <strong>
                {data.supplier ? data.supplier.legalName + ' · ' + data.supplier.ruc : 'Proveedor'}
              </strong>
          ...
          <div className="ticket-message supplier-message">
            <strong>Consulta del proveedor</strong>
            <p>{data.text}</p>
          </div>
          {data.answer ? (
            ...
            <form
              ...
              onSubmit={form.handleSubmit((values) => answer.mutate(values))}
            >
              <label>
                Responder
                <textarea
                  ...
                  placeholder="Escriba la respuesta para el proveedor"
                />
              </label>
              <button className="button primary" type="submit" disabled={answer.isPending}>
                <Check size={16} /> Enviar respuesta y cerrar
              </button>
            </form>
          )}
          {answer.isError && <p className="error-message">No se pudo enviar la respuesta.</p>}
        </section>
      )}
    </>
  );
  ...
  return (
    <>
      <PageHeader
        ...
        description="Las ofertas enviadas son visibles inmediatamente para usuarios autorizados."
      />
      <DataTable
        rows={data}
        columns={[
          ...
          },
          {
            ...
            header: 'Proveedor',
            render: (row) =>
              row.supplier ? row.supplier.ruc + ' - ' + row.supplier.legalName : '-',
          },
          ...
          { key: 'status', header: 'Estado', render: (row) => <StatusBadge status={row.status} /> },
          {
            key: 'submittedAt',
            header: 'Enviada (PY)',
            render: (row) => formatPyDateTime(row.submittedAt),
          },
          {
            key: 'action',
            header: '',
            render: (row) => (
              <div className="row-actions">
                <Link className="button ghost compact-button" to={'/internal/bids/' + row.id}>
                  Ver
                </Link>
                {hasRole(['ADMIN']) && (
                  <button
      ...
      {removeBid.isError && (
        <p className="error-message">
          {removeBid.error instanceof Error
            ? removeBid.error.message
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (bidToDelete) removeBid.mutate(bidToDelete.id);
        }}
      />
    </>
  );
...
export function BidDetailInternalPage() {
  ...
  return (
    <>
      <PageHeader
        title="Detalle interno de oferta"
        ...
        actions={
          <div className="row-actions">
            <button
              className="button ghost"
              type="button"
              onClick={() => data && previewBidA4(data)}
            >
              Previsualizar
            </button>
            <button
              className="button ghost"
              type="button"
              onClick={() => navigate('/internal/bids')}
            >
              Volver
            </button>
          </div>
        ...
        <>
          <section className="panel tender-detail">
            ...
            <dl className="detail-grid">
              ...
              <dt>Proveedor</dt>
              <dd>{data.supplier ? data.supplier.ruc + ' - ' + data.supplier.legalName : '-'}</dd>
              <dt>Enviada (PY)</dt>
...
export function AwardCancelDesertPage() {
  ...
  const formattedDecisionAmount =
    decisionCurrency === 'USD'
      ? new Intl.NumberFormat('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }).format(Number(decisionAmount) || 0)
      : new Intl.NumberFormat('es-PY', { maximumFractionDigits: 0 }).format(
          Math.round(Number(decisionAmount) || 0),
        );
  ...
  async function loadSuggestions(value: string, field: string) {
    ...
    try {
      setSuggestions(
        await api.get<DecisionSuggestion[]>(
          '/awards/suggestions?q=' + encodeURIComponent(value.trim()) + '&type=' + type,
        ),
      );
    } catch {
      setSuggestions([]);
  ...
  function selectSuggestion(item: DecisionSuggestion) {
    if (item.kind === 'tender') {
      setValue('tenderId', item.id);
      const code = item.code ? displayTenderCode(item.code) : item.label;
      setTenderDisplay(code);
      setIdentifier(code);
    }
    if (item.kind === 'supplier') {
      setValue('supplierId', item.id);
      const ruc = item.ruc ?? item.label;
      setSupplierDisplay(ruc);
      setIdentifier(ruc);
    }
    if (item.kind === 'bid') {
      setValue('bidId', item.id);
      if (item.tenderId) setValue('tenderId', item.tenderId);
      const code = item.tenderCode ? displayTenderCode(item.tenderCode) : item.label;
      setTenderDisplay(code);
      if (item.supplierId) setValue('supplierId', item.supplierId);
      if (item.supplierRuc) setSupplierDisplay(item.supplierRuc);
      const receipt = item.receiptCode ?? item.label.split(' - ')[0];
      setBidDisplay(receipt);
      if (item.amount !== undefined) setValue('amount', item.amount);
      setIdentifier(code);
    }
    setSuggestions([]);
    setSuggestionField('');
  ...
  function suggestionMenu(field: string) {
    return suggestionField === field && suggestions.length > 0 ? (
      <ul className="autocomplete-menu">
        {suggestions.map((item) => (
          <li key={item.kind + item.id}>
            ...
            >
              {item.kind === 'tender' && item.code
                ? displayTenderCode(item.code) + ' - ' + item.label.replace(/^.*? - /, '')
                : item.kind === 'bid' && item.tenderCode
                  ? item.label.replace(item.tenderCode, displayTenderCode(item.tenderCode))
                  : item.label}
            </button>
          </li>
        ))}
  ...
  async function resolveIdentifier(value: string) {
    ...
    try {
      const result = await api.get<AwardResolveResponse>(
        `/awards/resolve?identifier=${encodeURIComponent(value.trim())}`,
      );
  ...
  function selectEligibleBid(bid: {
    id: string;
    totalAmount: string;
    supplier?: { id: string };
    ...
    if (bid.tender?.id) {
      setValue('tenderId', bid.tender.id);
      setTenderDisplay(displayTenderCode(bid.tender.code));
    }
  }
            ...
            <div className="autocomplete-field">
              <input
                ...
                onChange={(e) => {
                  ...
                  void loadSuggestions(e.target.value, 'global');
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
        ...
        {resolveResult?.mode === 'single' && (
          <div className="resolve-preview">
            ...
            {resolveResult.supplier && (
              <div className="preview-row">
                <strong>Proveedor:</strong> {resolveResult.supplier.legalName} (RUC:{' '}
                {resolveResult.supplier.ruc})
              </div>
            )}
            {resolveResult.bid && (
              <div className="preview-row">
                <strong>Oferta:</strong> {resolveResult.bid.id} - {resolveResult.bid.status} -{' '}
                {formatMoney(resolveResult.bid.totalAmount, resolveResult.bid.currency)}
              </div>
            )}
              ...
              {resolveResult.options?.map((option) => (
                <li key={option.id}>
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setIdentifier(option.ruc);
                      void resolveIdentifier(option.ruc);
                    }}
                  >
                    {option.legalName} {option.tradeName ? `(${option.tradeName})` : ''} - RUC{' '}
                    {option.ruc}
                  </button>
                </li>
              ))}
        ...
        <form className="grid-form" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
          <label>
            ...
            <select {...register('mode')}>
              <option value="award">Adjudicar</option>
              <option value="cancel">Cancelar</option>
              <option value="desert">Desierta</option>
            </select>
          </label>
          <label>
            Licitacion ID
            ...
            <div className="autocomplete-field">
              <input
                value={tenderDisplay}
                placeholder="Ej: 026-001"
                onChange={(event) => {
                  setTenderDisplay(event.target.value);
                  setValue('tenderId', '');
                  void loadSuggestions(event.target.value, 'tender');
                }}
              />
              {suggestionMenu('tender')}
            </div>
          </label>
          <label>
            Proveedor ID
            ...
            <div className="autocomplete-field">
              <input
                ...
                onChange={(event) => {
                  setSupplierDisplay(event.target.value);
                  setValue('supplierId', '');
                  void loadSuggestions(event.target.value, 'supplier');
                }}
              />
              {suggestionMenu('supplier')}
            </div>
          </label>
          <label>
            Oferta ID
            <input type="hidden" {...register('bidId')} />
            <div className="autocomplete-field">
              <input
                value={bidDisplay}
                placeholder="Ej: REC-..."
                onChange={(event) => {
                  setBidDisplay(event.target.value);
                  setValue('bidId', '');
                  void loadSuggestions(event.target.value, 'bid');
                }}
              />
              {suggestionMenu('bid')}
            </div>
          ...
          <label>
            Monto ({decisionCurrency === 'USD' ? 'USD' : 'Gs.'})
            <input
              type="text"
              inputMode={decisionCurrency === 'USD' ? 'decimal' : 'numeric'}
              value={formattedDecisionAmount}
              onChange={(event) => {
                const raw =
                  decisionCurrency === 'USD'
...
export function RequestingAreasPage() {
  ...
  const create = useMutation({
    mutationFn: (values: { name: string; description: string }) =>
      api.post('/requesting-areas', values),
    onSuccess: () => {
  ...
  const update = useMutation({
    ...
    }) => api.patch('/requesting-areas/' + id, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['requesting-areas'] });
      closeForm();
    },
  ...
  const toggleStatus = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      api.patch('/requesting-areas/' + id, { status }),
    ...
    <>
      ...
      {showForm && (
        <section className="panel form-panel">
          <div className="section-heading">
            <div>
              <h2>{editingArea ? 'Editar area' : 'Nueva area solicitante'}</h2>
            </div>
              ...
              <div className="row-actions">
                ...
                <button
                  ...
                  onClick={() =>
                    toggleStatus.mutate({
                      id: String(row.id),
                      status: row.status === 'ACTIVA' ? 'INACTIVA' : 'ACTIVA',
                    })
                  }
                >
                  {row.status === 'ACTIVA' ? (
