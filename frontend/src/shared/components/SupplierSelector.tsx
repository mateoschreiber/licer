...
export function SupplierSelector({ value, onChange, label = 'Proveedor vinculado' }: SupplierSelectorProps) {
  ...
  return (
    <label>{label}<div className="autocomplete-field">
      <input value={text} placeholder="Buscar por RUC o nombre" autoComplete="off"
        onFocus={() => setOpen(Boolean(text.trim()))}
        onChange={(event) => { setText(event.target.value); setOpen(true); onChange(''); }}
        onKeyDown={(event) => { if (event.key === 'Enter' && matches[0]) { event.preventDefault(); select(matches[0]); } }}
      />
      {open && matches.length > 0 && <ul className="autocomplete-menu">{matches.map((supplier) => <li key={supplier.id}>
        <button type="button" onMouseDown={(event) => { event.preventDefault(); select(supplier); }}>
