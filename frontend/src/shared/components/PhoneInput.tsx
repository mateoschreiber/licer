...
function validCountry(value?: string | null): CountryCode {
  return countries.some((item) => item.code === value) ? (value as CountryCode) : 'PY';
}
function nationalNumber(value: string | null | undefined, country: CountryCode) {
...
export function PhoneInput({ value, country, onChange, onCountryChange, id }: PhoneInputProps) {
  ...
  const maxLength = Math.max(1, 15 - prefix.length + 1);
  useEffect(() => {
    const next = validCountry(country);
    setSelectedCountry(next);
    setNumber(nationalNumber(value, next));
  }, [value, country]);
  function changeCountry(next: CountryCode) {
    setSelectedCountry(next);
    onCountryChange?.(next);
    onChange('+' + getCountryCallingCode(next) + number.replace(/\D/g, ''));
  }
